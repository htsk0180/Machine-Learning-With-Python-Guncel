...
ln_reg.fit(x,y)
print(f"linear regression r-square değeri : {r2_score(Y, ln_reg.predict(X))}")
...
print(f"polynomial regression r-square değeri : {r2_score(Y, ln_reg4.predict(poly_reg4.fit_transform(X)))}")
...
print(f"support vector machine r-square değeri :  {r2_score(y_olcekli, svr_reg.predict(x_olcekli))}")
...
r_dt.fit(X,Y)
print(f"decision tree r-square değeri :  {r2_score(Y, r_dt.predict(X))}")
...
rf_reg.fit(X,Y)
print(f"random forest r-square değeri :  {r2_score(Y, rf_reg.predict(X))}")
...
# ALGORİTMALARIN DEĞERLENDİRİLMESİ:
from sklearn.metrics import r2_score
print(f"random forest r-square değeri :  {r2_score(Y, rf_reg.predict(X))}")
print(f"decision tree r-square değeri :  {r2_score(Y, r_dt.predict(X))}")
print(f"support vector machine r-square değeri :  {r2_score(y_olcekli, svr_reg.predict(x_olcekli))}")
